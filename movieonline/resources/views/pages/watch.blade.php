                                ...
                                <div class="halim-server">
                                    <ul class="halim-list-eps">
                                        @foreach ($movie->episode as $key => $sotap)
                                            <a href="{{ url('xem-phim/' . $movie->slug . '/tap-' . $sotap->episodede) }}">
                                                <li class="halim-episode"><span
                                                        class="halim-btn halim-btn-2 {{ $tapphim == $sotap->episodede ? 'active' : '' }}"
                                                        data-post-id="37976" data-server="1" data-episode="1"
                                                        data-position="first" data-embed="0"
                                                        data-title="Xem phim {{ $movie->title }} - Tập {{ $sotap->episodede }} - {{ $movie->name_eng }} - vietsub + Thuyết Minh"
                                                        data-h1="{{ $movie->title }} - {{ $sotap->episodede }}">{{ $sotap->episodede }}</span>
                                                </li>
                                            </a>
                                        @endforeach
